...
/***/ function(module, exports, __webpack_require__) {
	...
	__webpack_require__(/*! ./main.js */1);
	module.exports = __webpack_require__(/*! /Users/maxwellbates/Autodesk/genetic-constructor/extensions/construct-burst/main.js */1);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _assign = __webpack_require__(/*! babel-runtime/core-js/object/assign */ 3);
	...
	function assignChildren(block, state) {
	  var assigned = (0, _assign2.default)(block, {
	    components: block.components.map(function (compId) {
	      return (0, _assign2.default)({}, state.blocks[compId]);
	    })
	  });
	
	  if (assigned.components.length > 0) {
	    assigned.components.forEach(function (comp) {
	      return assignChildren(comp, state);
	    });
	  }
	...
	function drawSunburst(container, state, layout) {
	  var constructId = state.focus.constructId;
	  var construct = state.blocks[constructId];
	  var tree = assignChildren((0, _assign2.default)({}, construct), state);
	
	  var partition = d3.layout.partition().size([2 * Math.PI, layout.radius * layout.radius]).children(function (d) {
	    return d.components;
	  }).value(function (d) {
	    return d.sequence.length || 0;
	  });
	  ...
	  var path = d3.select(container).data([tree]).selectAll('path').data(nodes);
	
	  path.enter().append('svg:path')
	  //.attr('display', d => d.id !== constructId ? null : 'none') //hide root node
	  .attr('d', arc).style('fill', function (d) {
	    return d.metadata.color || '#cccccc';
	  }).style('stroke', 'transparent').style('opacity', 1).style('strokeWidth', '3px').on('click', function (d) {
	    return d.id !== constructId && window.constructor.api.focus.focusBlocks([d.id]);
	  });
	
	  path.transition(500).attr('d', arc).style('fill', function (d) {
	    return d.metadata.color || '#cccccc';
	  }).style('opacity', function (d) {
	    return isFocused(d) ? 0.75 : 1;
	  }).style('stroke', function (d) {
	    return isFocused(d) ? 'white' : 'transparent';
	  });
	...
	function render(container, options) {
	  ...
	  var layout = { radius: radius, width: width, height: height };
	  ...
	  var subscriber = window.constructor.store.subscribe(function onSubscribe(state, lastAction) {
